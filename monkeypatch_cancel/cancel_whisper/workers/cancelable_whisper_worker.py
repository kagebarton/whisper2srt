"""Cancelable whisper worker: per-encode-pass cancellation for stable-ts.

This is the core of the stable-ts cancellation proof-of-concept. Unlike the
audio-separator worker (which runs in a subprocess with Pipe-based cancel
signaling), stable-ts runs in the SAME process — so we can use a simple
threading.Event directly, no subprocess or Pipe bridging needed.

How it works:
1. The worker loads a faster-whisper model via stable_whisper.load_faster_whisper().
2. When align() is called, the worker MONKEY-PATCHES model.encode() to check
   a cancel_event before each encoder forward pass. model.encode() is the
   expensive GPU call that happens once per alignment iteration (batch of
   ~100 tokens) and once per refinement binary-search step.
3. The patched encode() checks cancel_event.is_set() — if set, it raises
   _CancelledInsideEncode instead of running the encoder.
4. The exception unwinds through:
   encode() → inference_func() → _compute_timestamps() → while loop →
   Aligner.align() → model.align()
   (or through Refiner._refine() → Refiner.refine() → model.refine())
5. The worker catches _CancelledInsideEncode, restores the original encode(),
   terminates any orphaned AudioLoader FFmpeg subprocesses, clears GPU state,
   and re-raises as AlignmentCancelledError.
6. The model weights (model.model, a CTranslate2 model) survive the exception
   — they're GPU/CPU attributes on the WhisperModel instance, not Python stack
   locals. The next align() call works immediately without reloading.

AudioLoader cleanup on cancellation:
When _CancelledInsideEncode unwinds through Aligner.align(), the normal
...
We fix this by:
a) Monkey-patching AudioLoader.__init__ to capture the FFmpeg subprocess's
   stderr at the OS level (redirect to /dev/null), so muxer errors from a
   killed process never reach the terminal.
b) In the _CancelledInsideEncode handler, walking the gc to find any
   orphaned AudioLoader instances and calling terminate() on them
   immediately — before FFmpeg stderr can bleed through.

Why patch model.encode() not something else?
- model.encode() is the single expensive GPU call shared by both align() and
  refine(). Patching it gives cancellation coverage for both operations with
  one patch point.
- model.encode() is a regular method (not a dunder), so instance-level
  monkey-patching works correctly (same reason we patch forward() not
  __call__() in the audio-separator case).
- model.encode() is called once per iteration of the Aligner's while loop
  (line 287 of non_whisper/alignment.py) and once per iteration of the
  Refiner's inner while loop (line 411 of non_whisper/refinement.py).

Cancel granularity:
- For align(): per-token-batch (~100 tokens ≈ ~5-10 seconds of audio,
  ~1-3s GPU time per batch)
- For refine(): per-binary-search-iteration (~0.1-0.5s GPU time per step)
  Refine actually has FINER granularity than align.

Same-process vs. subprocess:
- The audio-separator cancel_test needed a subprocess + Pipe bridging because
  audio-separator's demix() loop runs in the same process as the model.
  Since stable-ts also runs in-process, we could do the same. But there's a
  key difference: audio-separator's model is loaded in the WORKER subprocess,
  so the main process can kill the worker without losing its own state.
  With stable-ts, the model is loaded in the main process (or the only
  process), so killing it would lose the model. Our approach keeps everything
  in-process and uses exception-based cancellation instead.
"""
...
class CancelableWhisperWorker:
    """In-process stable-ts worker with per-encode-pass cancellation.

    Unlike CancelableStemWorker (which runs audio-separator in a subprocess),
    this worker runs stable-ts in the same process. Cancellation is done by
    ...
    def load_model(self) -> None:
        ...
        model_source = self._config.model_path or "faster-whisper-large-v3"
    ...
    def align(
        ...
        Returns:
            WhisperResult on success, None if cancelled.
    ...
    def align_and_refine(
        ...
        This matches the usage pattern in snippets/stable_align.py:
        model.align() followed by model.refine().

        Both operations share the same cancel_event and the same monkey-patch
        approach (model.encode() is patched independently for each call).
        ...
        # If the caller wants to cancel refine too, they set the event again.
