"""Cancelable stem worker: persistent subprocess with mid-separation cancellation.

This is the core of the cancellation proof-of-concept. The worker runs
audio-separator in a subprocess (model loaded once, stays loaded), but
...
4. When the orchestrator sets the cancel event, the main process forwards
...
class CancelableStemWorker:
    ...
    def start(self) -> None:
        ...
        self._process = Process(
            target=_cancelable_worker_main,
            args=(
                self._job_recv,
                self._result_send,
                self._cancel_recv,
        ...
        self._process.start()
        logger.info("Cancelable stem worker started (PID %d)", self._process.pid)
...
def _cancelable_worker_main(
    ...
    """Entry point for the cancelable stem worker subprocess.
    ...
    worker_log.info("Cancelable stem worker process started (PID %d)", os.getpid())
